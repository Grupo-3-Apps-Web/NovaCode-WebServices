...
public class ProfilesController(IProfileCommandService commandService, IProfileQueryService queryService) : ControllerBase
{
    [HttpGet]
    [SwaggerOperation("Get All Users", "Returns all registered user profiles.", OperationId = "GetAllUsers")]
    [SwaggerResponse(200, "Users found.", typeof(IEnumerable<ProfileResource>))]
    public async Task<IActionResult> GetAllUsers()
    {
        var query = new GetAllProfilesQuery();
        var users = await queryService.Handle(query);
    ...
    {
        var user = await queryService.Handle(new GetProfileByIdQuery(id));
    ...
    public async Task<IActionResult> CreateUser([FromBody] CreateProfileCommand command)
    {
        if (command == null)
            return BadRequest("Invalid data.");

        var created = await commandService.Handle(command);
        var resource = ProfileResourceFromEntityAssembler.ToResourceFromEntity(created);

        return CreatedAtAction(nameof(GetUserById), new { id = created.Id }, resource);
    }
    ...
    public async Task<IActionResult> UpdateUser(int id, [FromBody] UpdateProfileCommand command)
    {
        ...
        var updated = await commandService.Handle(id, command);
        if (updated == null)
            return NotFound($"Profile with ID {id} not found.");
    ...
    {
        var deleted = await commandService.Handle(new DeleteProfileCommand(id));
        if (deleted == null)
            return NotFound($"Profile with ID {id} not found.");
