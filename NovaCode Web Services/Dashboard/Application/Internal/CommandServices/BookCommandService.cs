...
{
    public async Task<Book?> Handle(int id, UpdateBookCommand command)
    {
        var book = await bookRepository.FindByIdAsync(id);
        ...
        book.UpdatePublication(command.Model,command.Brand, command.Year, command.Description, 
            command.Image, command.Price, command.Rating);
        try
        {
        ...
        {
            throw new Exception("Error updating book info");
        }
    }
